App = {
  ...
  bindEvents: function () {
    ...
    $(document).on("click", "#sign", function () {
      App.handleSignedMessage(
        jQuery("#receiver").val(), // Worker address
        jQuery("#amount").val() // Enter amount(cumulative)
      );
    });
    ...
    $(document).on("click", "#transfer", function () {
      App.handleTransfer(
        jQuery("#receiveramount").val(), // Enter amount(cumulative)
        jQuery("#signedMessage").val() // Enter signed message
      );
    });
    App.populateAddress();
  },
    ...
    new Web3(App.url).eth.getAccounts((err, accounts) => {
      if (!err) {
        ...
        App.web3.eth.getBalance(accounts[0]).then((res) => {
          jQuery("#sender_balance").text(App.web3.utils.fromWei(res), "ether");
        });
  ...
  signMessage: function (message, amount) {
    console.log('defaultAccount', App.defaultAccount)
